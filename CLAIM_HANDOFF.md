...
2. Resume from the local Slice 2I checkpoint at the head of `codex/claimant-submission-controller`, based on Slice 2H checkpoint `0215f30` after Slice 2G checkpoint `4e4d5c5`, Slice 2F checkpoint `9b59916`, and PR #65 merge commit `dcd6fefee4c527a4e0eceff54fed59e1f240f746`.
...
5. Phase 2 Slice 2I — concealed submission and safe-acknowledgement controller — is locally checkpointed. Its closing baseline is 969 passing workspace tests with 3 established mobile skips; all workspace typechecks, zero-warning lint, the 24-route production web build, API bundle, rollback-only submission database exercise, controller isolation, repository security, GitHub Actions security, and claimant-custody isolation passed.
6. Keep Slice 1G server, Slice 1H mobile, Slice 1I persistence, Slice 1J adapter/runtime, and Slice 2A through 2I approvals immutable `false`; normal app requests remain concealed, unmounted, or runtime-disconnected.
...
8. Start the next bounded slice with a hard-disabled, runtime-disconnected web submission coordinator using injected transport, strict request/acknowledgement validation, one stable idempotency key per attempt, bounded single-flight behavior, explicit abort/ambiguous-response retry semantics, and no browser persistence or normal-runtime import. Hosted migration, notification delivery, real evidence, and external access remain separate gates.
...
- Slice 1A hosted sign-in/MFA client wiring is parked at the paid-plan gate without weakening server enforcement. Slice 1B/1C, disposable custody-probe evidence, and the owner-accepted 2026-08-12 adversarial review/remediation are complete. Slice 1D-1H merged in PR #65; Slice 1I through Slice 2G have local checkpoints, Slice 2H is checkpointed at `0215f30`, and Slice 2I is checkpointed at the current branch head. Slice 1G/1H/1I/1J/2A/2B/2C/2D/2E/2F/2G/2H/2I approvals remain immutable false. Native Swift implementation/evidence, hosted MFA, provider selection, distributed edge abuse controls, and external activation remain gates.
