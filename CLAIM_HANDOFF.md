...
5. Phase 2 Slice 2G — hard-disabled dashboard read-model coordinator — is code-complete locally. Review its coherent safe-projection validation, case/version binding, stale/divergent response handling, cross-case clearing, hostile tests, and evidence before publishing.
6. Keep Slice 1G server, Slice 1H mobile, Slice 1I persistence, Slice 1J adapter/runtime, and Slice 2A through 2G approvals immutable `false`; normal app requests remain concealed or runtime-disconnected.
...
8. The next bounded slice is a hard-disabled server-owned submission and safe-acknowledgement transaction using synthetic data, exact case/evidence authority, idempotent replay, persisted receipt, and value-free audit/outbox writes. Its route, hosted migration, notification delivery, real evidence, and external access remain separate gates.
...
- Slice 1A hosted sign-in/MFA client wiring is parked at the paid-plan gate without weakening server enforcement. Slice 1B/1C, disposable custody-probe evidence, and the owner-accepted 2026-08-12 adversarial review/remediation are complete. Slice 1D-1H merged in PR #65; Slice 1I through Slice 2F have local checkpoints, and Slice 2G is code-complete locally. Slice 1G/1H/1I/1J/2A/2B/2C/2D/2E/2F/2G approvals remain immutable false. Native Swift implementation/evidence, hosted MFA, provider selection, distributed edge abuse controls, and external activation remain gates.
