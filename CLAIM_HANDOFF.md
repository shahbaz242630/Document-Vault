...
2. Resume from `codex/claimant-upload-controller`, based on local Slice 2D checkpoint `5fb3cb8` after Slice 2C checkpoint `b7b6557`, Slice 2B checkpoint `2b4f748`, and PR #65 merge commit `dcd6fefee4c527a4e0eceff54fed59e1f240f746`.
...
5. Phase 2 Slice 2H — server-owned submission and safe-acknowledgement transaction — is code-complete locally. Review its portal/key/case/intake/preparation/clean-object authority, atomic receipt/audit/outbox/idempotency writes, rollback/replay tests, and evidence before publishing.
6. Keep Slice 1G server, Slice 1H mobile, Slice 1I persistence, Slice 1J adapter/runtime, and Slice 2A through 2H approvals immutable `false`; normal app requests remain concealed, unmounted, or runtime-disconnected.
...
8. The next bounded slice is a concealed immutable-false submission/acknowledgement controller with exact origin/body/idempotency rules, verified bearer-derived identity, fresh AAL2, active portal assertion, server-derived authority, generic failures, and bounded concurrency. Hosted migration, notification delivery, real evidence, and external access remain separate gates.
...
- Phase 2 Slice 2H adds one hard-disabled, unmounted server-owned submission transaction. It locks and reasserts exact portal, claimant, current-key, case/version, policy, intake, latest preparation, checklist disposition, consumed-capability, and clean-object authority before atomically advancing to `submitted` and writing an append-only safe receipt, value-free audit/outbox events, and idempotency result. Replay is stable, changed input fails, and late failures fully roll back. The acknowledgement explicitly keeps review and release false. Evidence: `docs/verification/2026-08-12-claimant-slice-2h-submission-acknowledgement-transaction.md`.
