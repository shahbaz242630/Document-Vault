...
2. Resume from `codex/claimant-intake-foundation`, based on local Slice 1J checkpoint `9cb50b2` after Slice 1I checkpoint `e8a73ae` and PR #65 merge commit `dcd6fefee4c527a4e0eceff54fed59e1f240f746`.
...
4. Phase 2 Slice 2A — claim-intake/checklist persistence — is code-complete locally. Review its exact case/claimant/policy binding, atomic `draft` to `identity_pending` transition, hostile tests, and evidence before publishing.
5. Keep Slice 1G server, Slice 1H mobile, Slice 1I persistence, Slice 1J adapter/runtime, and Slice 2A intake approvals immutable `false`; normal app routes cannot reach them.
...
7. The next bounded non-native slice is checklist progress/evidence-metadata persistence and server-owned transitions. Keep binary upload for a later separately reviewed private-quarantine Storage/malware-adapter slice.
...
- Slice 1A hosted sign-in/MFA client wiring is parked at the paid-plan gate without weakening server enforcement. Slice 1B/1C, disposable custody-probe evidence, and the owner-accepted 2026-08-12 adversarial review/remediation are complete. Slice 1D-1H merged in PR #65; Slice 1I and Slice 1J have local checkpoints, and Slice 2A is code-complete locally. Slice 1G/1H/1I/1J/2A approvals remain immutable false. Native Swift implementation/evidence, hosted MFA, and edge abuse controls remain activation gates.
