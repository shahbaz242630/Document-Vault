...
2. Resume from `codex/claimant-upload-processor`, based on local Slice 2C checkpoint `b7b6557` after Slice 2B checkpoint `2b4f748` and PR #65 merge commit `dcd6fefee4c527a4e0eceff54fed59e1f240f746`.
...
4. Phase 2 Slice 2D — streaming upload/reconciliation processor — is code-complete locally. Review its stream bounds, atomic abandonment, ambiguous-commit preservation, scanner retry, hostile tests, and evidence before publishing.
5. Keep Slice 1G server, Slice 1H mobile, Slice 1I persistence, Slice 1J adapter/runtime, Slice 2A intake, Slice 2B preparation, Slice 2C quarantine, and Slice 2D processor approvals immutable `false`; normal app routes cannot reach them.
...
7. The next bounded slice is a concealed, fresh-AAL2 upload controller around the processor with exact origin/content-type/body/concurrency rules and deterministic local adapters. Real Storage/scanner provider selection and external access remain separate gates.
...
- Slice 1A hosted sign-in/MFA client wiring is parked at the paid-plan gate without weakening server enforcement. Slice 1B/1C, disposable custody-probe evidence, and the owner-accepted 2026-08-12 adversarial review/remediation are complete. Slice 1D-1H merged in PR #65; Slice 1I through Slice 2C have local checkpoints, and Slice 2D is code-complete locally. Slice 1G/1H/1I/1J/2A/2B/2C/2D approvals remain immutable false. Native Swift implementation/evidence, hosted MFA, and edge abuse controls remain activation gates.
