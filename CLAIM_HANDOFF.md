...
2. Resume from the completed Slice 3G branch on `codex/claimant-review-escalation-foundation`, based on Slice 3F `7bb6862`, Slice 3E `f5a9728`, Slice 3D `9089497`, Slice 3C `f8d6211`, Slice 3B `2aebfa7`, Slice 3A `d752de1`, and the earlier claimant checkpoints.
...
5. Phase 3 Slice 3G — review escalation and appeal persistence — is complete locally. Its closing baseline is 1,021 passing workspace tests with 3 established mobile skips; 141 static/security tests, typecheck, lint, build, bundle, rollback database, and isolation checks passed without changing hosted Supabase or downloading Supabase images.
6. Keep Slice 1G server, Slice 1H mobile, Slice 1I persistence, Slice 1J adapter/runtime, Slice 2A through 2J, and Slice 3A/3B/3C/3D/3E/3F/3G approvals immutable `false`; normal app requests remain concealed, unmounted, or runtime-disconnected.
...
8. Begin Phase 4 Slice 4A: add a separately reviewed immutable-false, unmounted, service-only release-eligibility and final-authorization foundation. Revalidate every current case/cycle/review/policy/key prerequisite, require no open intervention, and fail closed without package creation or retrieval authority. Shahbaz Malik remains the accountable human test reviewer and Codex a non-human technical test actor only. Add no reviewer UI/evidence access, provider, hosted migration, real data, deployment, ciphertext/package creation, native behavior, or external behavior.
