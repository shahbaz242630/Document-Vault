...
2. Resume from the code-complete uncommitted Slice 3B working tree on `codex/claimant-owner-notice-delivery` at Slice 3A checkpoint `d752de1`, following Slice 2J `e9853fb`, Slice 2I `f8935c2`, Slice 2H `0215f30`, Slice 2G `4e4d5c5`, Slice 2F `9b59916`, and PR #65 merge `dcd6fefee4c527a4e0eceff54fed59e1f240f746`.
...
5. Phase 3 Slice 3B — hard-disabled unmounted owner-notice delivery coordinator — is code-complete. Its closing baseline is 990 passing workspace tests with 3 established mobile skips; typecheck, lint, build, bundle, security, and isolation checks passed without touching hosted Supabase or Docker.
6. Keep Slice 1G server, Slice 1H mobile, Slice 1I persistence, Slice 1J adapter/runtime, Slice 2A through 2J, and Slice 3A/3B approvals immutable `false`; normal app requests remain concealed, unmounted, or runtime-disconnected.
...
8. Add a hard-disabled service-only outbox lease/claim/completion/reconciliation persistence boundary that persists stable dispatch/idempotency keys before contact and reclaims leases without redispatch. Add no provider/network call, route, hosted migration, owner UI, release predicate, real data, or external behavior.
