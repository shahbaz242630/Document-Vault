...
2. Resume from the completed Slice 3F branch on `codex/claimant-independent-review-foundation`, based on Slice 3E `f5a9728`, Slice 3D `9089497`, Slice 3C `f8d6211`, Slice 3B `2aebfa7`, Slice 3A `d752de1`, and the earlier claimant checkpoints.
...
5. Phase 3 Slice 3F — independent review decisions and two-person approval enforcement — is complete locally. Its closing baseline is 1,015 passing workspace tests with 3 established mobile skips; 135 static/security tests, typecheck, lint, build, bundle, rollback database, and isolation checks passed without changing hosted Supabase.
6. Keep Slice 1G server, Slice 1H mobile, Slice 1I persistence, Slice 1J adapter/runtime, Slice 2A through 2J, and Slice 3A/3B/3C/3D/3E/3F approvals immutable `false`; normal app requests remain concealed, unmounted, or runtime-disconnected.
...
8. Begin Phase 3 Slice 3G: add an immutable-false, unmounted, service-only escalation and appeal persistence foundation. Require explicit hold behavior, separate synthetic authority, immutable safe reasons/events, current-version locking/idempotency, and invalidation of any prior review aggregate. Shahbaz Malik remains the accountable human test reviewer and Codex a non-human technical test actor only. Keep release authorization false and add no reviewer UI/evidence access, provider, hosted migration, real data, deployment, package creation, or external behavior.
