...
**Repository integration gate:** publish the Phase 1 files from clean branch `codex/mvp-api-frankfurt` through a focused pull request to `main`. The live production deployment was built from the local workspace, so a later Git-driven deployment from unchanged `main` could return the API to `iad1`. Verify the pull request is merged before beginning MVP Phase 2.
