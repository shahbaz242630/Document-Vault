...
2026-06-08 Android QA update:

- Fresh emulator launch exposed a Metro bundling blocker: Expo Router was picking up `.test.ts` files under `apps/mobile/app`, then trying to bundle Node-only imports such as `node:fs`.
- Moved the route source tests out of the Expo Router app tree and into `apps/mobile/src/app-route-tests`.
- Verification passed:
...
- Opening Settings > Emergency access then exposed a native render crash: `Cannot read property 'from' of undefined`.
- Root cause was detaching Supabase client's `from` method during repository construction, which lost the client's `this` context in the real runtime.
- Updated `createSupabaseEmergencyGrantRepository` to call `client.from(...)` directly and added a regression test that requires bound client context.
- Verification passed:
  ...
  - `npm run typecheck --workspace @vault/mobile`
- Android manual QA then reached the sealed-code creation action. The screen loads, acknowledgement enables the create button, and the app shows a generic failure alert instead of crashing.
- Runtime log shows the actual live-project blocker: `Could not find the table 'public.emergency_key_grants' in the schema cache`.
- Local migration `supabase/migrations/20260603210500_emergency_access_foundation.sql` exists, but the linked Supabase project is not yet migrated through that file.
- `supabase migration list` could not verify remote state because the cached linked database password failed authentication. The next testing step is to run `supabase db push --linked` with the current Supabase database password, then retry sealed-code creation on Android.
- The remote Supabase project was migrated through `20260603210500_emergency_access_foundation.sql`, and PostgREST schema reload was notified through a disposable Docker `postgres:16-alpine` psql container.
- Retesting then exposed a second backend mismatch: mobile writes sealed-code audit events, but `audit_events_event_type_check` did not allow `sealed_emergency_code_created`, `sealed_emergency_code_revoked`, or `sealed_emergency_code_regenerated`.
- Added and applied `supabase/migrations/20260608200000_add_sealed_emergency_code_audit_event_types.sql`, then notified PostgREST schema reload again.
- Verification passed:
  ...
  - `npm run typecheck --workspace @vault/mobile`
  - `supabase migration list --linked` shows local/remote both at `20260608200000`.
- Retesting then exposed one more runtime blocker: `new row violates row-level security policy for table "emergency_key_grants"`.
- Root cause was the Emergency access route creating a fresh Supabase client, while the unlocked vault uses the authenticated Supabase client handed off during sign-in. The fresh client had no authenticated session on Android, so RLS rejected emergency grant inserts.
- Updated the vault session context to retain the authenticated Supabase client, clear it on lock/sign-out, and use it for emergency grant repository creation.
- Verification passed:
  ...
  - `npm run typecheck --workspace @vault/mobile`
- Android manual QA completed the sealed-code creation flow after signing in with the fixed authenticated-client handoff.
- The one-time code screen displayed the required copy: `Write this code down now. Sanduqkin cannot show it again after you confirm.`
- After confirming the code was written down, the UI changed to `Emergency code active` and showed `Sanduqkin no longer has the raw code. Regenerate if you need a new copy.`
- The raw one-time code was no longer present in the Android UI tree after confirmation.

Recommended next slice:

- Start from the verified Android state above; do not rebuild the sealed-code creation flow unless regression testing requires it.
- Test the interrupted one-time-code path:
  - create or regenerate a sealed emergency code,
  - leave the one-time code page before confirmation or restart the app,
  - verify the app shows the interrupted setup state,
  - verify the raw code is not recovered from storage,
  - verify regenerate replaces the unusable interrupted code.
- Then test revoke/regenerate from the active sealed-code state and confirm Supabase stores only encrypted/wrapped grant material plus status metadata.
- Keep using slice discipline: implement/test/confirm/update handoff before moving to the next emergency-access area.
