...
Combined Slices 5X–5Z pass local security verification on `codex/claimant-runtime-foundations`, stacked on Slice 5W head `f352c9a`. The hosted-identity boundary requires synchronous fresh non-recovery AAL2 with exact issuer/audience; the native-shaped boundary requires the exact test alias/fingerprint, verified user presence and non-exportable private-key custody; the 5Z wrapper is separately literal-false and statically isolated. Outputs remain value-free and failure closes authority. Full local tests, typechecks, zero-warning lint, repository/workflow security, secret/dependency/vector/custody checks, related isolation gates, API bundle and web build pass. No ambient authentication/network/native/provider access, hosted mutation, deployment, real data, merge or activation is authorized. Exact-head CI, preview and logs must be owned by the dedicated watcher, with only narrow repair/resubmission on confirmed red.

## Current checkpoint — Slice 5W local acceptance, 2026-09-15

Slice 5W is published in PR #87 with an independent literal-false gate and a dedicated static isolation guard. It is the sole approved wrapper around the merged 5U–5V session journey, accepts only the exact frozen value-free draft, runs only existing local synthetic submission/state-machine contracts, reconciles the synthetic ledger and exposes no identifier, token, challenge, signature, private binding or runtime authority. Full local tests, typechecks, zero-warning lint, repository/GitHub Actions security, dependency audit, claim-vector/custody checks, API bundle and web build pass. The active green/red watcher owns exact-head CI/preview/log evidence and narrow repair/resubmission on confirmed red; it cannot merge, weaken controls or start another slice. No hosted mutation, production authentication/custody, provider contact, deployment, real data or capability activation occurred.
...
Slice 5T in stacked PR #85 adds only an isolated literal-false synthetic claimant-portal session client. It fixes exact request paths/origins/headers/body, accepts only bounded private responses, retains one fresh non-recovery AAL2 session in memory, preserves server session version, restricts retry to ambiguous activate/revoke, and suppresses late local state commit after cancellation/disposal. Static guards reject ambient auth/network/Supabase/native/storage adapters and normal runtime importers. Final local focused/full tests, typechecks, lint, Phase 1, security, workflow, secret, dependency, API bundle, web build and related isolation gates pass. The active `pr-84-85-claimant-green-merge` watcher owns sequential exact-head gating and merge. No hosted mutation, production authentication, native custody, deployment, real claimant data or authority activation is permitted. Evidence: `docs/verification/2026-09-15-claimant-slice-5t-portal-session-client.md`.

## Current checkpoint — Slice 5S local composition, 2026-09-15

Slice 5S in stacked PR #84 adds only an isolated, literal-false synthetic claimant-session-to-bridge harness. It freezes exact fresh-AAL2 claimant user/session/version binding, closes permanently on drift or hostile lifecycle ordering, clears retry/draft authority on background or cancellation, suppresses late results and reduces completion to a value-free draft summary without identifiers, bearer material, proof source or replay detail. Static guards reject activation, ambient authentication/network/native/storage adapters and normal runtime importers. Final local focused/full tests, typechecks, lint, Phase 1, security, workflow, secret, dependency, API bundle, web build and isolation gates pass. The active ten-minute `pr-83-84-claimant-5s-gates` watcher monitors exact-head CI/preview evidence and reports green/red without merging. No hosted mutation, production signer/custody, deployment, real claimant data or authority activation is permitted. Evidence: `docs/verification/2026-09-15-claimant-slice-5s-session-bridge-composition.md`.
...
This supersedes older PR/watcher/next-action snapshots below. PR #71 and PR #72 are merged; Slice 5P is local on `codex/claimant-offline-code-v2-handoff-client`, based on `96e41e0`. New client approvals are literal false, with no normal-runtime importer, ambient authentication/network/storage or native signer.

The client checks the server transcript's domain, exact claimant/session/version, source challenge, record binding and expiry before synthetic injected signing. It transmits only existing allowlisted request fields and a header bearer, and retains only the exact signed completion and public binding metadata for bounded retries. The server remains independently authoritative. A draft result grants no identity, intake, review or release authority.

Verification and limitations: `docs/verification/2026-09-05-claimant-slice-5p-handoff-client.md`. Use a watcher for exact-head PR gates and protected preview smoke/logs: green permits progression, red requires focused repair and re-evaluation. Do not merge pending/red checks or inaccessible staging, weaken protection, mutate hosted systems, activate capabilities, or start a further slice automatically. Preserve `.codex-runtime/` and `.playwright-cli/` untouched.
...
6. Done 2026-09-25: the `image-size` patch/exception is retired; Metro 0.84.5 removes the dependency and the audit has no allowlist. Any new high/critical advisory needs a fresh documented review.
