...
- Active branch: `codex/claimant-evidence-metadata`, based on local Slice 2A checkpoint `2c73a33` after Slice 1J checkpoint `9cb50b2` and PR #65 merge `dcd6fefee4c527a4e0eceff54fed59e1f240f746`.
...
1. Review the local Slice 2B evidence-preparation metadata increment and its evidence; publish only with explicit owner authorization.
2. Keep Slice 1G/1H/1I/1J/2A/2B hard-disabled and unreachable from normal app routes. Do not deploy the local claimant migrations or enable a claimant capability.
...
4. Review and bound the next private-quarantine upload slice around randomized case-bound capabilities, Storage RLS, file parsing limits, malware scanning, outage behavior, retention/deletion, and hostile authorization before adding binary handling.
