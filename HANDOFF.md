...
- Active branch: `codex/claimant-dashboard-read-model`, based on Slice 2F checkpoint `9b59916` after Slice 2E checkpoint `cd67838` and PR #65 merge `dcd6fefee4c527a4e0eceff54fed59e1f240f746`.
...
1. Review the local Slice 2G dashboard read-model coordinator and its evidence; publish only with explicit owner authorization.
2. Keep Slice 1G/1H/1I/1J/2A/2B/2C/2D/2E/2F/2G hard-disabled and concealed or disconnected from normal app requests. Do not deploy the local claimant/Storage migrations or enable a claimant capability.
...
4. Bound the next slice around a hard-disabled server-owned submission and safe-acknowledgement transaction with synthetic data, exact case/evidence authority, idempotent replay, persisted receipt, and value-free audit/outbox writes. Keep the API route, hosted migration, notifications, real evidence, and external behavior separately gated.
