...
- Active branch: `codex/claimant-encrypted-package-delivery`; Slice 4E is complete locally on top of Slice 4D `f0195fe`, Slice 4C `ef97a64`, Slice 4B `8d9d195`, Slice 4A `f4dcdaf`, and the earlier claimant checkpoints.
...
1. Begin Phase 4 Slice 4F with an immutable-false, runtime-disconnected mobile encrypted-package custody/open coordinator. Verify the exact delivered payload and signed manifest, use only an injected production-shaped native custody adapter for claimant-device decryption, and keep local open distinct from delivery and export.
2. Keep Slice 1G/1H/1I/1J/2A/2B/2C/2D/2E/2F/2G/2H/2I/2J/3A/3B/3C/3D/3E/3F/3G/4A/4B/4C/4D/4E hard-disabled and concealed, unmounted, or disconnected from normal app requests. Do not deploy the local claimant/Storage migrations or enable a claimant capability.
...
4. In Slice 4F, treat Shahbaz Malik as the accountable human test reviewer and Codex only as a non-human technical test actor. Add no reviewer UI/evidence access, browser plaintext, server decryption, public/signed URL, production Swift binding, hosted migration, real data, deployment, export, or external behavior.
