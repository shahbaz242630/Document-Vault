...
1. Review the local Slice 2D upload/reconciliation processor and its evidence; publish only with explicit owner authorization.
2. Keep Slice 1G/1H/1I/1J/2A/2B/2C/2D hard-disabled and unreachable from normal app routes. Do not deploy the local claimant/Storage migrations or enable a claimant capability.
...
4. Bound the next slice around a concealed fresh-AAL2 upload controller and deterministic local adapters; do not select or configure a real Storage/scanner provider without separate authorization.
