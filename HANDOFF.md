...
- Active branch: `codex/claimant-submission-transaction`, based on Slice 2G checkpoint `4e4d5c5` after Slice 2F checkpoint `9b59916` and PR #65 merge `dcd6fefee4c527a4e0eceff54fed59e1f240f746`.
...
1. Review the local Slice 2H submission/safe-acknowledgement transaction and its evidence; publish only with explicit owner authorization.
2. Keep Slice 1G/1H/1I/1J/2A/2B/2C/2D/2E/2F/2G/2H hard-disabled and concealed, unmounted, or disconnected from normal app requests. Do not deploy the local claimant/Storage migrations or enable a claimant capability.
...
4. Bound the next slice around a concealed immutable-false submission/acknowledgement controller with exact origin/body/idempotency rules, verified identity, fresh AAL2, active portal assertion, generic failures, and bounded concurrency. Keep hosted migration, notifications, real evidence, and external behavior separately gated.
