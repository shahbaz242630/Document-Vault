...
- Active branch: `codex/claimant-upload-processor`, based on local Slice 2C checkpoint `b7b6557` after Slice 2B checkpoint `2b4f748` and PR #65 merge `dcd6fefee4c527a4e0eceff54fed59e1f240f746`.
...
1. Review the local Slice 2E upload controller/synthetic adapters and its evidence; publish only with explicit owner authorization.
2. Keep Slice 1G/1H/1I/1J/2A/2B/2C/2D/2E hard-disabled and concealed from normal app requests. Do not deploy the local claimant/Storage migrations or enable a claimant capability.
...
4. Bound the next slice around a hard-disabled claimant evidence-upload client coordinator with synthetic fixtures, injected transport, cancel/progress/retry/reconciliation, and no browser persistence. Do not select or configure a real Storage/scanner provider or accept real files without separate authorization.
