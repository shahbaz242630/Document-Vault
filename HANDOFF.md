...
- Active branch: `codex/claimant-private-quarantine`, based on local Slice 2B checkpoint `2b4f748` after Slice 2A checkpoint `2c73a33` and PR #65 merge `dcd6fefee4c527a4e0eceff54fed59e1f240f746`.
...
1. Review the local Slice 2C private-quarantine foundation and its evidence; publish only with explicit owner authorization.
2. Keep Slice 1G/1H/1I/1J/2A/2B/2C hard-disabled and unreachable from normal app routes. Do not deploy the local claimant/Storage migrations or enable a claimant capability.
...
4. Bound the next slice around a hard-disabled streaming upload/reconciliation processor using the injected Storage/scanner contracts; do not select or configure a real provider without separate authorization.
