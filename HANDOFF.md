...
- Active branch: `codex/claimant-upload-client-coordinator`, based on Slice 2E checkpoint `cd67838` after Slice 2D checkpoint `5fb3cb8` and PR #65 merge `dcd6fefee4c527a4e0eceff54fed59e1f240f746`.
...
1. Review the local Slice 2F upload client coordinator and its evidence; publish only with explicit owner authorization.
2. Keep Slice 1G/1H/1I/1J/2A/2B/2C/2D/2E/2F hard-disabled and concealed or disconnected from normal app requests. Do not deploy the local claimant/Storage migrations or enable a claimant capability.
...
4. Bound the next slice around a hard-disabled claimant dashboard/read-model coordinator with synthetic safe projections, injected transport, server-authoritative state, generic failures, and no sensitive browser persistence. Do not select or configure a real provider or enable hosted/external behavior without separate authorization.
