...
- Active branch: `codex/claimant-retrieval-session-foundation`; Slice 4D is complete locally on top of Slice 4C `ef97a64`, Slice 4B `8d9d195`, Slice 4A `f4dcdaf`, Slice 3G `bc61110`, and the earlier claimant checkpoints.
...
1. Begin Phase 4 Slice 4E with an immutable-false, unmounted encrypted-package delivery transaction/coordinator. Consume one exact unexpired 4D authorization, revalidate portal/finalization/grant/key authority, record `package_served` only after complete ciphertext delivery, and advance `release_ready` to `released` only on the first successful delivery.
2. Keep Slice 1G/1H/1I/1J/2A/2B/2C/2D/2E/2F/2G/2H/2I/2J/3A/3B/3C/3D/3E/3F/3G/4A/4B/4C/4D hard-disabled and concealed, unmounted, or disconnected from normal app requests. Do not deploy the local claimant/Storage migrations or enable a claimant capability.
...
4. In Slice 4E, treat Shahbaz Malik as the accountable human test reviewer and Codex only as a non-human technical test actor. Add no reviewer UI/evidence access, browser plaintext, server decryption, public/signed URL, hosted migration, real data, deployment, native behavior, or external behavior.
