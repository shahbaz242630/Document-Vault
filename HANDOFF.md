...
- Active branch: `codex/claimant-intake-foundation`, based on local Slice 1J checkpoint `9cb50b2` after Slice 1I checkpoint `e8a73ae` and PR #65 merge `dcd6fefee4c527a4e0eceff54fed59e1f240f746`.
...
1. Review the local Slice 2A intake/checklist increment and its evidence; publish only with explicit owner authorization.
2. Keep Slice 1G/1H/1I/1J/2A hard-disabled and unreachable from normal app routes. Do not deploy the local claimant migrations or enable a claimant capability.
...
4. Bound the next local non-native slice around checklist progress/evidence metadata and server-owned transitions; defer binary upload to a separate private-quarantine Storage/malware-adapter slice.
