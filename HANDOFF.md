...
- Active branch: `codex/claimant-signed-manifest-foundation`; Slice 4C is complete locally on top of Slice 4B `8d9d195`, Slice 4A `f4dcdaf`, Slice 3G `bc61110`, Slice 3F `7bb6862`, Slice 3E `f5a9728`, Slice 3D `9089497`, Slice 3C `f8d6211`, Slice 3B `2aebfa7`, Slice 3A `d752de1`, and the earlier claimant checkpoints.
...
1. Begin Phase 4 Slice 4D with an immutable-false, unmounted, service-only claimant retrieval-session authorization foundation. Require fresh AAL2 and an active claimant portal context, bind a short-lived single-purpose session to the exact current release-ready finalization and claimant key/grant, and keep package serving/decryption/native opening disabled.
2. Keep Slice 1G/1H/1I/1J/2A/2B/2C/2D/2E/2F/2G/2H/2I/2J/3A/3B/3C/3D/3E/3F/3G/4A/4B/4C hard-disabled and concealed, unmounted, or disconnected from normal app requests. Do not deploy the local claimant/Storage migrations or enable a claimant capability.
...
4. In Slice 4D, treat Shahbaz Malik as the accountable human test reviewer and Codex only as a non-human technical test actor. Add no reviewer UI/evidence access, provider, hosted migration, real data, deployment, package serving, decryption, native behavior, or external behavior.
5. Continue afterward through claimant-addressed encrypted package preparation and native retrieval, and only then the offline-code V2 route.
