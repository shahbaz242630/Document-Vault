...
describe("offline-code V2 persistence service", () => {
  it("is immutable-false for every operation", async () => {
    expect(CLAIMANT_OFFLINE_CODE_V2_PERSISTENCE_APPROVED).toBe(false);
    const value = service();
    for (const operation of [value.register(registration()),
      value.issueChallenge(challenge()), value.recordAttempt(attempt()),
      value.revoke(revocation())]) {
  ...
  it("passes only strict server-owned synthetic persistence inputs", async () => {
    ...
    const value = service({ approved: true, transactions });
    await value.register(registration());
    await value.issueChallenge(challenge());
    await value.recordAttempt(attempt());
    await value.revoke(revocation());
  ...
  it("rejects V1, raw locator/secret/private material, unsafe outcomes, and extra fields", async () => {
    const value = service({ approved: true });
    for (const hostile of [
      { ...registration(), protocol: "sanduqkin:claim:offline-code:v1" },
      { ...registration(), locator: "SK2-L-prohibited" },
      { ...registration(), clientSecret: "SK2-S-prohibited" },
      { ...registration(), proofPrivateKey: "prohibited" },
      { ...registration(), mek: "prohibited" },
      { ...challenge(), claimantId: id("99") },
      { ...attempt(), verificationOutcome: "release" },
      { ...revocation(), expectedLocatorVersion: 1 },
    ]) {
      const operation = "challengeId" in hostile
        ? ("verificationOutcome" in hostile
          ? value.recordAttempt(hostile) : value.issueChallenge(hostile))
...
function service(options: Record<string, unknown> = {}) {
  return createOfflineCodeV2PersistenceService({
