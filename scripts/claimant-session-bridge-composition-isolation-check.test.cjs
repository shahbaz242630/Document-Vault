...
const { validateSources, path } = require("./claimant-session-bridge-composition-isolation-check.cjs");

const baseline = collectSources(join(__dirname, ".."));

test("session bridge composition remains disabled and isolated", () => validateSources(baseline));
...
test("rejects a normal application importer", () => {
  ...
  sources.set("apps/mobile/app/claim.tsx",
    'import { createClaimantSessionBridgeComposition } from "../src/features/claimant-handoff/session-bridge-composition";');
