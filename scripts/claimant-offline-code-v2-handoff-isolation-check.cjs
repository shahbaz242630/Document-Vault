...
for (const token of ["offline-code-v2-handoff-routes",
  "/claimant/offline-code/v2/handoffs/issue", "/claimant/offline-code/v2/handoffs/complete"])
  if (!index.includes(token)) throw new Error(`Authenticated handoff runtime mount is missing: ${token}`);
if (!routes.includes("CLAIMANT_OFFLINE_CODE_V2_HANDOFF_ROUTES_APPROVED = false as const"))
  throw new Error("Authenticated handoff runtime approval is not literal false.");
if (!routes.includes("createService: () =>") || !routes.includes("requireClaimantCapability"))
  throw new Error("Authenticated handoff runtime dependencies are not lazily capability-gated.");
...
for (const token of ["proof_private_key", "client_secret", "plaintext_mek", "root_secret", "wrap_key"])
  if (migration.includes(token) || routes.includes(token) || sources.some(([, source]) => source.includes(token)))
